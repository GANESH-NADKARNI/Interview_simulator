...
export default function Register() {
  ...
  const handleResend = async () => {
    ...
    try { await authApi.resendOtp({ email: form.email, type: 'VERIFY_EMAIL' }); setResendTimer(60); setOtp(['','','','','','']); toast.success('New OTP sent!') }
    ...
    <div style={cs}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <form onSubmit={handleRegister} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[{ key: 'username', label: 'Username', type: 'text', ph: 'Choose a username' }, { key: 'email', label: 'Email', type: 'email', ph: 'your@email.com' }, { key: 'password', label: 'Password', type: 'password', ph: 'Min 8 characters' }].map(({ key, label, type, ph }) => (
            <div key={key}>
  ...
  return (
    <div style={cs}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <div style={{ ...logo, background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.2)' }}><Mail size={26} color="#00d4ff" /></div>
