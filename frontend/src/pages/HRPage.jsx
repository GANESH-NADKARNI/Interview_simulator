...
import { Mic, MicOff, Volume2, ChevronRight, RotateCcw, Send, AlertTriangle, Edit2, Check } from 'lucide-react'
...
export default function HRPage() {
  ...
  const isInterviewActive = state === 'interview'

  useEffect(() => {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition
    if (!SpeechRecognition) setSpeechSupported(false)
    return () => { stopRecording(); synthRef.current?.cancel() }
  }, [])
  ...
  const startRecording = () => {
    ...
    let finalText = transcript.trim() ? transcript.trim() + ' ' : ''
    ...
    timerRef.current = setInterval(() => setRecordSeconds(Math.floor((Date.now() - startTimeRef.current) / 1000)), 1000)
  }
  ...
  const submitAnswer = async () => {
    if (!transcript.trim()) return toast.error('Please record or type your answer first')
    stopRecording()
    setIsEditing(false)
    setAnalyzing(true)
    ...
    try {
      const { data } = await hrApi.analyze({ question: questions[current], transcript: transcript.trim(), duration })
      ...
      if (current < questions.length - 1) {
        ...
        // Collect expression summary before finishing
    ...
    <div className="fade-in" style={{ maxWidth: 700, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', padding: '60px 20px' }}>
        ...
        <p style={{ color: 'var(--text2)', marginBottom: 12, fontSize: 15 }}>
          5 HR questions with voice recording, editable transcript, and live expression analysis via your camera.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
          {['Voice Recording', 'Editable Transcript', '📷 Expression Analysis', 'Tone Analysis', 'Grammar Check'].map(t => (
            <span key={t} className="tag tag-medium">{t}</span>
          ))}
        </div>
        {!speechSupported && (
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', padding: 14, borderRadius: 12, marginBottom: 20, background: 'rgba(255,68,102,0.1)', border: '1px solid rgba(255,68,102,0.2)', color: 'var(--red)', fontSize: 13 }}>
            <AlertTriangle size={16} />
            Speech recognition not supported. Use Chrome for voice. You can type your answers directly.
          </div>
        )}
  ...
  if (state === 'interview' && questions.length > 0) {
    ...
    return (
      <>
        <ExpressionTracker ref={expressionRef} isActive={isInterviewActive} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                {!isRecording && transcript.trim() && (
                  <button onClick={() => setIsEditing(!isEditing)}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, border: '1px solid var(--border2)', background: isEditing ? 'rgba(0,212,255,0.1)' : 'var(--bg)', color: isEditing ? 'var(--accent)' : 'var(--text3)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                    {isEditing ? <><Check size={13} /> Done Editing</> : <><Edit2 size={13} /> Edit</>}
                  </button>
                )}
              </div>
            </div>

            {isEditing || !speechSupported ? (
              <textarea ref={textareaRef} value={transcript} onChange={e => setTranscript(e.target.value)}
                placeholder={speechSupported ? 'Edit your transcribed answer...' : 'Type your answer here...'}
                style={{ width: '100%', minHeight: 140, background: 'var(--bg)', borderRadius: 10, padding: 16, marginBottom: 16, border: '1px solid var(--accent)', fontSize: 14, lineHeight: 1.8, color: 'var(--text)', fontFamily: 'var(--font)', resize: 'vertical', boxSizing: 'border-box' }}
              />
            ) : (
              <div style={{ minHeight: 120, background: 'var(--bg)', borderRadius: 10, padding: 16, marginBottom: 16, border: isRecording ? '1px solid rgba(255,68,102,0.3)' : '1px solid var(--border2)', fontSize: 14, lineHeight: 1.8, color: transcript ? 'var(--text)' : 'var(--text3)', whiteSpace: 'pre-wrap' }}>
                {transcript || 'Your spoken answer will appear here...'}
                {isRecording && !transcript && (
                  <div style={{ display: 'flex', gap: 4, marginTop: 8 }}>
                    {[1, 2, 3].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--red)', animation: `pulse ${0.5 + i * 0.2}s ease-in-out infinite` }} />)}
                  </div>
                )}
              </div>
            )}

            {!isRecording && transcript.trim() && !isEditing && speechSupported && (
              <p style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 12, textAlign: 'center' }}>
                💡 Speech-to-text not perfect? Click <strong>Edit</strong> to fix your answer before submitting.
              </p>
            )}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {speechSupported && (
                <button onClick={isRecording ? handleStopRecording : startRecording}
                  ...
                  {isRecording ? <><MicOff size={16} /> Stop Recording</> : <><Mic size={16} /> {transcript ? 'Continue Recording' : 'Start Recording'}</>}
                </button>
              )}
              <button className="btn btn-primary" onClick={submitAnswer}
                disabled={analyzing || !transcript.trim() || isRecording}
